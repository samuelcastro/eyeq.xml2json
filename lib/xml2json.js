...
xml2json.prototype.convert = function(files, program, cb) {
    for(var i in files) {
        parser.parseString(fs.readFileSync(files[i]), function(err, result) {
            if(err) cb(err);
            if (filendir.ws(program.destination + '/' + files[i].replace(program.origin, ''), JSON.stringify(result, null, "\t"))) {
                cb(null, { file: program.destination + '/' + files[i].replace(program.origin, ''), output: result, index: i * 1 })
            }
        });
    }
};
